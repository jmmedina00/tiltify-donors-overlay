...
describe("LinkGenerator", () => {
  ...
  global.navigator.clipboard = {
    writeText: async contents => {clipboardContents = contents;}
  };
  ...
  test("creates a link with the data input from the form (1)", () => {
    ...
    expect(clipboardContents).toBe("localhost?campaign=1234&token=abcd&currency=euro&swap=on");
  });
  ...
  test("creates a link with the data input from the form (2)", () => {
    ...
    // Host is "localhost" during unit tests
    expect(clipboardContents).toBe("localhost?campaign=3456&token=1a2b&currency=dollar&highest=on");
  });
})