...
describe("Tiltify service", () => {
  ...
  const server = setupServer(
    rest.get("https://tiltify.com/api/v3/campaigns/:campaignId/donations", (req, res, ctx) => {
      const { params: { campaignId }, headers: { map: { authorization: auth } } } = req;
      ...
      const response = Array.isArray(selectedCampaign) ? selectedCampaign[before] : selectedCampaign;
  ...
  test("throws an Error if token isn't accepted (the most likely thing to go wrong)", async () => {
    await expect(getDonations(tokenFail, "1234")).rejects.toThrow("Token has been banned");
  });
});